...
bool Manager::checkForCollisions() {
  ...
  while ( sprite != sprites.end() ) {
    if ( player->collidedWith(*sprite) ) {
        ...
        sprites.push_back(new ExplodingSprite(*(*sprite)));
        (*sprite)->respawn();
        return true;
    }
    ++sprite;
...
void Manager::draw() const {
  ...
  for(std::list<ExplodingSprite*>::const_iterator it = exploding.begin(); it != exploding.end(); it++) {
    (*it)->draw();
  }
...
void Manager::update() {
  ...
  for(std::list<ExplodingSprite*>::iterator it = exploding.begin(); it != exploding.end(); it++) {
    if((*it)->chunkCount() == 0){
      it = exploding.erase(it);
    }
  }
