...
Clock::Clock() :
  ...
  frames(0),
  fpsmax(Gamedata::getInstance().getXmlInt("frameMax")),
  fps(new double[fpsmax]), 
  ndx(0),
  tickSum(0),
  sumOfAllTicks(0),
...
double Clock::getFPS() {
    fps[ndx%fpsmax] = (double) SDL_GetTicks()/1000;
    ndx++;
    if(ndx == fpsmax) {
        ndx = 0;
        return (double)fpsmax/(fps[fpsmax-1] - fps[0]);
    } else {
        return (double)fpsmax/(fps[ndx-1] - fps[ndx]);
    }
}
